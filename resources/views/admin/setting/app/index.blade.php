                                ...
                                <tbody>
                                    ...
                                    @if($app != null)
                                        <tr >
                                            ...
                                            <td>{{ 1  }}</td>
                                            <td>
                                             {{\Illuminate\Support\Str::limit($app->content, 50, '...')}}
                                           </td>
                                            <td>{{ \Carbon\Carbon::parse($app->created_at)->format('g:i A') }}  </td>

                                            <td>
                                                <div class="action-btn">

                                                     <a href="{{ url( $locale .'/about_app/'. $app->id .'/edit' )}}" id="btn-edit"
                                                       class="btn btn-info btn-xs"><i class="material-icons">edit</i></a>
                                                        ...
                                                        <button class="btn btn-danger btn-flat btn-sm remove-user" type="submit"
                                                            onclick="deleteConfirmation({{$app->id}})"> <i class="material-icons">
                                                           delete </i></button>

                                                     <form
                                                        id="delete-form-{{ $app->id }}" action="{{ route('about_app.destroy', $app->id) }}" class="delete_btn"
